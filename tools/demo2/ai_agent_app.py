...
with tab1:
    ...
    if uploaded_file is not None:
        ...
        netlist_content = st.text_area(
            ...
            height=300,
            placeholder="module your_module(rst_n, clk, ...)\n  // Your netlist code\nendmodule"
        )
...
with tab5:
    ...
    if explain_mode == "ðŸŽ¯ Explain Netlist":
        ...
        col1, col2 = st.columns(2)
        with col1:
            module1 = st.text_input("First module name:", placeholder="e.g., your_module")
        with col2:
...
with st.sidebar:
    ...
    **EDA Netlist AI Analyzer**
    ...
    # Quick load your_netlist.v
    if st.button("ðŸ“‚ Load your_netlist.v", use_container_width=True):
        try:
            netlist_path = "../your_netlist.v"
            if os.path.exists(netlist_path):
                with open(netlist_path, 'r') as f:
                    st.session_state['loaded_netlist'] = f.read()
                st.success("âœ… your_netlist.v loaded!")
                st.rerun()
            else:
    ...
    st.markdown("---")
    st.caption("ðŸ’¡ **Tip**: Load your your_netlist.v using the button above, then explore different analysis tabs!")
